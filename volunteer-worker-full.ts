...
function generateId() {
  // ফরম্যাট: NOHANA-XXXXXX (৬ সংখ্যা) — উদাহরণ: NOHANA-521080
...
<p style="font-size:13px;color:#555;background:#fef3c7;padding:8px;border-radius:6px">
  ID ফরম্যাট: <b>৬ অক্ষর + স্পেস + সংখ্যা</b><br>
  উদাহরণ: <code>NOHANA 502160</code> বা <code>NAHANA 505263</code>
</p>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
  ...
  <div>
    <label>কর্মচারী ID * (৬ অক্ষর + সংখ্যা)</label>
    <input id="emp_id" placeholder="যেমন: NOHANA 502160" style="text-transform:uppercase">
  </div>
  <div>
...
<hr>
<h4>নির্বাচিত কর্মচারীর নোট হিস্ট্রি (Conversation)</h4>
<div id="noteHistory" style="background:#f8fafc;padding:12px;border-radius:8px;min-height:80px">
  কোনো কর্মচারী সিলেক্ট করুন বা Edit বাটন চাপুন
</div>
</div>
...
function showTab(t){
  ...
  ['volTab','examTab','linksTab','empTab'].forEach(id=>document.getElementById(id).style.display='none');
  ...
  if(t=='emp'){document.getElementById('empTab').style.display='block'; document.querySelectorAll('.tab')[3].classList.add('active'); loadEmployees();}
}
...
async function saveEmployee(){
  ...
  let empId = document.getElementById('emp_id').value.trim().toUpperCase();
  ...
  // ৬ অক্ষর চেক (স্পেস বাদে)
  const letters = empId.replace(/[^A-Z]/g,'');
  if(letters.length < 6) return alert('ID-তে অন্তত ৬টি অক্ষর থাকতে হবে। উদাহরণ: NOHANA 502160');
  ...
  document.getElementById('empList').innerHTML = '<table><tr><th>নাম (বাঁদিকে)</th><th>ID</th><th>পদ</th><th>মোবাইল</th><th>সর্বশেষ নোট</th><th>তারিখ</th><th>Action</th></tr>' +
    j.data.map(e => \`
      <tr>
        ...
        <td>
          <button onclick="fillEmpForm('\${e.emp_id}','\${e.name}','\${e.post||''}','\${e.mobile||''}')">Edit + নোট</button>
          <button onclick="loadNoteHistory('\${e.emp_id}')">হিস্ট্রি</button>
          <button onclick="delEmployee('\${e.emp_id}')" style="background:#dc2626">X</button>
        </td>
      </tr>
    \`).join('') + '</table>';
}
...
async function loadNoteHistory(empId){
  ...
  try{
    ...
    document.getElementById('noteHistory').innerHTML = '<b>'+empId+'</b> এর নোট হিস্ট্রি:<br><br>' +
      j.data.map(n => \`
        <div class="note-box">
...
async function delEmployee(id){
  ...
  loadEmployees();
  document.getElementById('noteHistory').innerHTML = 'কোনো কর্মচারী সিলেক্ট করুন';
}
    ...
    if (url.pathname === "/api/admin/employees") {
      ...
      if (req.method === "POST") {
        try {
          ...
          const empId = String(d.emp_id).trim().toUpperCase();
