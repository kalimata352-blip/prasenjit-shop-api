...
function gradeToPoint(g){return {A:4,B:3,C:2,D:1}[g]||0}
function calcPercent(u){
  const grades=[u.bengali,u.english,u.history,u.geography,u.math,u.gk];
  const avg=grades.reduce((s,g)=>s+gradeToPoint(g),0)/grades.length;
...
async function search(){
  ...
  const resultText = percent>=60 ? 'পাস (Pass)' : 'ফেল (Fail)';
  const resultColor = percent>=60 ? '#16a34a' : '#dc2626';
  el.innerHTML=\`
    <h3 style="text-align:center">📜 \${u.name} - রেজাল্ট</h3>
    <p><b>ID:</b> \${u.id} | <b>ফোন:</b> \${u.mobile}</p>
    <table>
      <tr><th>বিষয়</th><th>গ্রেড (A/B/C/D)</th></tr>
      ...
      <tr style="font-weight:bold;background:#f0fdf4"><td>পার্সেন্টেজ (অটো)</td><td><span style="font-size:18px;color:#16a34a">\${percent}%</span></td></tr>
      <tr style="font-weight:bold;background:#fef2f2"><td>রেজাল্ট</td><td><span style="font-size:18px;color:\${resultColor}">\${resultText}</span></td></tr>
    </table>
...
<p style="color:#16a34a;font-size:13px">⭐ গ্রেড সিলেক্ট করলেই <b>পার্সেন্টেজ ও মোট গ্রেড অটো</b> হিসাব হবে (A=4, B=3, C=2, D=1 পয়েন্ট)</p>
...
<div><label>বাংলা</label><select id="e_ben" onchange="calcTotal()"><option>A</option><option>B</option><option>C</option><option>D</option></select></div>
<div><label>ইংরেজি</label><select id="e_eng" onchange="calcTotal()"><option>A</option><option>B</option><option>C</option><option>D</option></select></div>
<div><label>ইতিহাস</label><select id="e_his" onchange="calcTotal()"><option>A</option><option>B</option><option>C</option><option>D</option></select></div>
<div><label>ভূগোল</label><select id="e_geo" onchange="calcTotal()"><option>A</option><option>B</option><option>C</option><option>D</option></select></div>
<div><label>অংক</label><select id="e_math" onchange="calcTotal()"><option>A</option><option>B</option><option>C</option><option>D</option></select></div>
<div><label>GK</label><select id="e_gk" onchange="calcTotal()"><option>A</option><option>B</option><option>C</option><option>D</option></select></div>
</div>
<div style="margin-top:15px;padding:12px;background:#f0fdf4;border-radius:8px;border:1px solid #bbf7d0">
  <b>অটো রেজাল্ট:</b> মোট গ্রেড = <span id="e_total_display" style="font-size:20px;color:#000">A</span> &nbsp;|&nbsp; 
  পার্সেন্টেজ = <span id="autoPercent" style="font-size:20px;color:#16a34a">100%</span> &nbsp;|&nbsp;
  রেজাল্ট = <span id="autoResult" style="font-size:18px;color:#16a34a">পাস</span>
  <input type="hidden" id="e_total" value="A">
</div>
...
function calcTotal(){
  ...
  const points=ids.map(id=>gradeToPoint(document.getElementById(id).value));
  const avg=points.reduce((a,b)=>a+b,0)/points.length;
  const totalG=pointToGrade(avg);
...
function fillExamForm(id,name,mobile){
  ...
  // default A for all, then calc
  ['e_ben','e_eng','e_his','e_geo','e_math','e_gk'].forEach(id=>{document.getElementById(id).value='A'});
  calcTotal();
}
async function saveExam(){
  calcTotal(); // ensure latest
  const data={
  ...
  const j=await r.json();
  document.getElementById('examMsg').innerHTML=j.success?'✅ '+j.message+' (পার্সেন্টেজ অটো সেভ হয়েছে)':'❌ '+j.message;
...
async function loadExamList(){
  ...
  document.getElementById('examList').innerHTML='<table><tr><th>ID</th><th>নাম</th><th>বাং</th><th>ইং</th><th>ইতি</th><th>ভূগো</th><th>অংক</th><th>GK</th><th>মোট</th><th>%</th><th>Del</th></tr>'+j.data.map(e=>{
    const pts=[e.bengali,e.english,e.history,e.geography,e.math,e.gk].map(g=>({A:4,B:3,C:2,D:1}[g]||0));
