...
function generateId() {
  // ফরম্যাট: নোহনা -XXXXXX (৬ সংখ্যা) — উদাহরণ: নোহনা -521080
  const num = Math.floor(100000 + Math.random() * 900000); // 100000–999999
  return `নোহনা -${num}`;
}

// কর্মচারী নোট / লিংক ইত্যাদিতে ইউনিক আইডি
...
<p style="font-size:13px;color:#555;background:#fef3c7;padding:8px;border-radius:6px">
  ...
  উদাহরণ ID: <code>নোহনা -521080</code> বা <code>NOHANA 502160</code>
</p>
...
<div style="margin-bottom:12px">
  ...
  <small style="color:#666">ID লিখে Enter চাপলেও লোড হবে</small>
</div>
...
<hr>
<h4>সব কর্মচারী (নাম বাঁদিকে • ID ডানদিকে)</h4>
<button onclick="loadEmployees()" style="margin-bottom:10px">রিফ্রেশ করুন</button>
<div id="empList"></div>

<hr>
<h4>নির্বাচিত কর্মচারীর পুরনো নোট (Conversation)</h4>
...
function showTab(t){
  try{
    ...
    if(t=='vol'){const el=document.getElementById('volTab'); if(el)el.style.display='block'; if(tabs[0])tabs[0].classList.add('active');}
    if(t=='exam'){const el=document.getElementById('examTab'); if(el)el.style.display='block'; if(tabs[1])tabs[1].classList.add('active'); loadExamList();}
    if(t=='links'){const el=document.getElementById('linksTab'); if(el)el.style.display='block'; if(tabs[2])tabs[2].classList.add('active'); loadLinksAdmin();}
    if(t=='emp'){const el=document.getElementById('empTab'); if(el)el.style.display='block'; if(tabs[3])tabs[3].classList.add('active'); loadEmployees();}
  }catch(err){console.error('showTab error',err); alert('Tab error: '+err.message);}
}
...
async function loadVolunteers(){
  ...
  document.getElementById('vlist').innerHTML='<table><tr><th>ID</th><th>নাম</th><th>মোবাইল</th><th>ভোটার</th><th>District</th><th>Status</th><th>Action</th></tr>'+j.data.map(v=>{
    return \`<tr><td>\${v.id}</td><td>\${v.name}</td><td>\${v.mobile}</td><td>\${v.voter_card||'-'}</td><td>\${v.district||''}</td><td>\${v.status}</td><td><button onclick="updateStatus('\${v.id}','approved')">Approve</button><button onclick="updateStatus('\${v.id}','rejected')">Reject</button><button onclick="fillExamForm('\${v.id}','\${v.name}','\${v.mobile}')">Exam</button></td></tr>\`
  }).join('')+'</table>';
}
...
async function loadEmployeeById(){
  ...
  try{
    // সব কর্মচারী থেকে খুঁজে বের করো
    ...
    if(found){
      ...
      // নতুন কর্মচারী — ফর্ম খালি রাখো, শুধু নোট হিস্ট্রি খালি দেখাও
...
async function loadEmployees(){
  ...
  document.getElementById('empList').innerHTML = '<table><tr><th>নাম (বাঁদিকে)</th><th>ID</th><th>পদ</th><th>মোবাইল</th><th>সর্বশেষ নোট</th><th>তারিখ</th><th>Action</th></tr>' +
    j.data.map(e => {
      const safeId = encodeURIComponent(e.emp_id || '');
      const safeName = (e.name || '').replace(/'/g, "\\\\'");
      const safePost = (e.post || '').replace(/'/g, "\\\\'");
      const safeMobile = (e.mobile || '').replace(/'/g, "\\\\'");
      return \`
      <tr>
        <td><b>\${e.name||''}</b></td>
        ...
        <td>
          <button onclick="fillEmpForm(decodeURIComponent('\${safeId}'),'\${safeName}','\${safePost}','\${safeMobile}')">Edit + নোট</button>
          <button onclick="loadNoteHistory(decodeURIComponent('\${safeId}'))">হিস্ট্রি</button>
          <button onclick="delEmployee(decodeURIComponent('\${safeId}'))" style="background:#dc2626">X</button>
        </td>
      </tr>\`;
    }).join('') + '</table>';
}
...
async function loadNoteHistory(empId){
  ...
  try{
    const r = await fetch('/api/admin/employees/'+encodeURIComponent(empId)+'/notes',{headers:{'X-Admin-Token':ADMIN_TOKEN}});
    const j = await r.json();
    if(!j.success){document.getElementById('noteHistory').innerHTML='❌ '+j.message; return;}
    if(!j.data || j.data.length===0){
      document.getElementById('noteHistory').innerHTML = '<p>এই কর্মচারীর কোনো নোট নেই</p>';
      return;
    }
    document.getElementById('noteHistory').innerHTML = '<b>'+empId+'</b> এর পুরনো নোট:<br><br>' +
      j.data.map(n => \`
        <div class="note-box">
      ...
      \`).join('');
  }catch(e){document.getElementById('noteHistory').innerHTML='❌ Error';}
}
...
// ID ইনপুটে Enter চাপলে লোড (script body-র শেষে থাকে বলে সরাসরি attach)
(function(){
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      // ===== নতুন টেবিল: কর্মচারী + নোট হিস্ট্রি =====
    ...
    if (url.pathname === "/api/admin/employees") {
      ...
      if (req.method === "POST") {
        try {
          ...
          const empId = String(d.emp_id).trim(); // বাংলা/ইংরেজি দুটোই রাখার জন্য uppercase করা হয়নি
          const note = (d.note || "").trim();

          // employees টেবিলে upsert
          ...
          // নোট থাকলে employee_notes-এ যোগ করো (conversation)
          if (note) {
