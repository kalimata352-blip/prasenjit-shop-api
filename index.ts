...
// ============ HTML TEMPLATES - ORIGINAL UNCHANGED LOGIC WITH SECURITY FIX ============

const HTML_PAGE = `<!DOCTYPE html><html lang="bn"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Prasenjit Shop - Volunteer Registration</title><style>
...
#adminFloat{position:fixed;right:14px;bottom:14px;background:#111;color:#fff;padding:8px 12px;border-radius:20px;text-decoration:none;font-size:13px}
</style></head><body>
<div class="card">
<h2 style="text-align:center">ভলেন্টিয়ার ফর্ম - তথ্য দিয়ে রেজিস্ট্রেশন</h2>
<div class="top-links">
...
f.addEventListener('submit',async e=>{
 ...
 try{
  ...
  if(j.success){
   msg.innerHTML='✅ রেজিস্ট্রেশন সফল! আপনার ID: <b>'+j.id+'</b><br><a target="_blank" href="/api/volunteer/pdf/'+j.id+'">PDF দেখুন</a>';
   f.reset();
   loadLinks();
...
async function loadLinks(){
 try{
  ...
  el.innerHTML=j.data.map(x=>{
   ...
   const preview = isImage ? \`<div style="margin-top:6px"><img src="\${x.link}" style="max-width:100%;border-radius:8px;max-height:200px"></div>\` : '';
   const icon = isPdf ? '📄' : (isImage ? '🖼️' : '🔗');
   return \`<div style="border:1px solid #eee;padding:10px;border-radius:8px;margin-bottom:8px">
   <div><b>\${x.title}</b><br><small>\${new Date(x.created_at).toLocaleDateString('bn-IN')}</small></div>
   \${preview}
   <div style="display:flex;gap:6px;margin-top:6px">
   <a class="btn" href="\${x.link}" target="_blank">\${icon} দেখুন</a>
   <a class="btn" style="background:#0d6efd;color:#fff" href="\${x.link}"\${downloadAttr}>⬇️ ডাউনলোড</a>
   </div></div>\`
  }).join('');
...
const STATUS_PAGE = `<!DOCTYPE html><html lang="bn"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Status Check</title><style>body{font-family:system-ui;padding:16px;background:#f5f5f5}.card{max-width:500px;margin:auto;background:#fff;padding:20px;border-radius:12px}input{width:100%;padding:10px;margin:6px 0}</style></head><body><div class="card"><h2>ID / মোবাইল দিয়ে স্ট্যাটাস চেক করুন</h2><input id="q" placeholder="ID বা মোবাইল নম্বর লিখুন"><button onclick="check()">চেক করুন</button><div id="res" style="margin-top:12px"></div><p><a href="/">হোম</a> | <a href="/links">নোটিফিকেশন</a></p></div><script>
async function check(){
 ...
 if(!j.success) res.innerHTML='❌ '+j.message;
  ...
  const d=j.data;
  res.innerHTML='<b>নাম:</b> '+d.name+'<br><b>মোবাইল:</b> '+d.mobile+'<br><b>স্ট্যাটাস:</b> <b style="color:'+(d.status=='approved'?'green':(d.status=='rejected'?'red':'orange'))+'">'+d.status+'</b><br><br><a target="_blank" href="/api/volunteer/pdf/'+d.id+'">PDF দেখুন</a>';
 }
}
</script></body></html>`;

const LINKS_PAGE = `<!DOCTYPE html><html lang="bn"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Notifications</title><style>body{font-family:system-ui;padding:16px;background:#f5f5f5}.card{max-width:700px;margin:auto;background:#fff;padding:20px;border-radius:12px}.btn{padding:8px 14px;background:#eee;border-radius:8px;text-decoration:none;color:#111;display:inline-block}</style></head><body><div class="card"><h2>গুরুত্বপূর্ণ লিঙ্ক ও নোটিফিকেশন</h2><div id="list">লোড হচ্ছে...</div><p><a href="/">হোম</a> 🏠</p></div><script>
async function load(){
 const r=await fetch('/api/links'); const j=await r.json();
 ...
 el.innerHTML=j.data.map(x=>{
  ...
  <a class="btn" style="background:#198754;color:#fff" href="\${x.link}"\${downloadAttr}>ডাউনলোড</a>
  </span></div>\`.join('');
 }
load();
</script></body></html>`;

const ADMIN_PAGE = `<!DOCTYPE html><html lang="bn"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Admin Panel</title><style>body{font-family:system-ui;padding:16px;background:#f5f5f5}.card{max-width:900px;margin:auto;background:#fff;padding:20px;border-radius:12px}input{padding:8px;margin:4px;border:1px solid #ccc;border-radius:6px}button{padding:8px 14px;border:0;background:#111;color:#fff;border-radius:6px;cursor:pointer}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border:1px solid #ddd;padding:6px;font-size:13px}.danger{background:#dc3545}</style></head><body><div class="card"><h2>Admin Panel - Prasenjit Shop</h2>
<div><input id="token" type="password" placeholder="Admin Token লিখুন" style="width:70%"><button onclick="saveToken()">Save</button>
<br><small style="color:#666">টিপস: টোকেন ব্রাউজারে সেভ থাকবে, কেউ দেখতে পাবে না।</small>
</div><hr>
<h3>Volunteers</h3>
...
<br><small style="display:block;margin:6px 0;color:#666">টিপস: যদি কোনো চিঠি/নোটিশ আপলোড করতে চান নিচের ফাইল আপলোড ব্যবহার করুন</small>
...
<input id="pdftitle" placeholder="টাইটেল দিন">
...
async function loadVolunteers(){
 ...
 document.getElementById('vlist').innerHTML='<table><tr><th>ID</th><th>নাম</th><th>মোবাইল</th><th>স্ট্যাটাস</th><th>Action</th></tr>'+j.data.map(v=>\`<tr><td>\${v.id}</td><td>\${v.name}</td><td>\${v.mobile}</td><td>\${v.status}</td><td>
...
async function loadLinksAdmin(){
 ...
 document.getElementById('linkList').innerHTML=j.data.map(x=>\`<div style="border-bottom:1px solid #eee;padding:6px 0"><span><b>\${x.title}</b> - \${x.link}</span><span><a href="\${x.link}" target="_blank"> দেখুন</a> | <a href="#" onclick="delLink('\${x.id}');return false">Delete</a></span></div>\`).join('');
}
async function addLink(){
 ...
 const r=await fetch('/api/admin/links',{method:'POST',headers:{'Content-Type':'application/json','X-Admin-Token':ADMIN_TOKEN},body:JSON.stringify({id:generateId(),title,link})});
...
async function uploadPdf(){
 ...
 reader.onload=async()=>{
  ...
  const r=await fetch('/api/admin/upload',{method:'POST',headers:{'Content-Type':'application/json','X-Admin-Token':ADMIN_TOKEN},body:JSON.stringify({title: title||file.name, filename:file.name, data:base64})});
  const j=await r.json();
  if(j.success){document.getElementById('uploadMsg').innerHTML='✅ আপলোড সফল! লিঙ্ক: <a href="'+j.link+'" target="_blank">'+j.link+'</a>'; document.getElementById('llink').value=j.link; document.getElementById('ltitle').value=title||file.name;}
...
const PDF_PAGE = (v: any) => `<!DOCTYPE html><html lang="bn"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Volunteer PDF</title><style>
...
.btn{padding:10px 18px;border-radius:6px;text-decoration:none;display:inline-block;cursor:pointer;border:0}
.btn-print{background:#0d6efd;color:#fff}
.btn-download{background:#198754;color:#fff}
@media print{.actions{display:none}}
</style></head><body>
<div class="card" id="pdfContent">
<h2>ভলেন্টিয়ার তথ্য - ${v.name}</h2>
<p style="text-align:center">ID: ${v.id} | Date: ${new Date().toLocaleDateString('bn-IN')}</p>
...
<br><p style="text-align:center">এই তথ্যটি যাচাই করা হয়েছে।</p>
</div>
<div class="actions">
 ...
 async fetch(req: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === "/api/links") {
   ...
   try {
    let results: any;
    if (cat === "all") {
     // Hide volunteer private PDFs from public
    ...
    return new Response(JSON.stringify({ success: true, data: combined }), { headers });
   } catch (e: any) {
  ...
  if (url.pathname.startsWith("/api/file/")) {
   ...
   if (!q) return new Response(JSON.stringify({ success: false, message: "ID পাওয়া যায়নি" }), { headers });
   const row: any = await env.DB.prepare("SELECT * FROM public_files WHERE id = ?").bind(q).first();
   if (!row) return new Response(JSON.stringify({ success: false, message: "ফাইল পাওয়া যায়নি" }), { headers });
   ...
   try {
    ...
    return new Response(JSON.stringify({ success: false, message: "File decode error" }), { headers });
   }
  }
  ...
  if (url.pathname.startsWith("/api/volunteer/status/") && req.method === "GET") {
   ...
   if (!row) return Response.json({ success: false, message: "তথ্য পাওয়া যায়নি" }, { headers });
  ...
  if (url.pathname === "/api/admin/upload" && req.method === "POST") {
   ...
   const id = generateId();
   await env.DB.prepare("INSERT INTO public_files (id, title, filename, data, created_at) VALUES (?,?,?,?,?)").bind(id, body.title||body.filename, body.filename||body.title, body.data, Date.now()).run();
   await env.DB.prepare("INSERT INTO public_links (id, title, link, created_at) VALUES (?,?,?,?)").bind(id, body.title||body.filename, `/api/file/${id}`, Date.now()).run();
