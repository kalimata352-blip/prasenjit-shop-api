...
f.addEventListener('submit',async e=>{
  ...
  try{
    ...
    if(j.success){
      msg.innerHTML='<span style="color:green">✅ রেজিস্ট্রেশন সম্পন্ন! আপনার ID: <b>'+j.id+'</b><br><a target="_blank" href="/api/volunteer/pdf/'+encodeURIComponent(j.id)+'">PDF দেখুন</a></span>';
      f.reset();
      loadLinks();
    } else {
...
async function check(){
  ...
  const d=j.data;
  res.innerHTML='<b>নাম:</b> '+d.name+'<br><b>মোবাইল:</b> '+d.mobile+'<br><b>ভোটার কার্ড:</b> '+(d.voter_card||'-')+'<br><b>স্ট্যাটাস:</b> <b style="color:'+(d.status=='approved'?'green':'orange')+'">'+d.status+'</b><br><a href="/api/volunteer/pdf/'+encodeURIComponent(d.id)+'">PDF দেখুন</a> | <a href="/result?id='+encodeURIComponent(d.id)+'">Result দেখুন</a>';
}
</script></body></html>`;
...
async function submitQuiz(){
  ...
  try{
    ...
    box.innerHTML=\`
      ...
      <p>রেজাল্ট অটো সেভ হয়েছে। <a href="/result?id=\${encodeURIComponent(currentVol.id)}">Result দেখুন</a> | <a href="/status">Status চেক</a></p>
    \`;
...
.error{color:#dc2626;background:#fef2f2;padding:10px;border-radius:8px;margin:8px 0}
.success{color:#16a34a;background:#f0fdf4;padding:10px;border-radius:8px;margin:8px 0}
</style>
</head><body>
...
<div id="tokenStatus" style="margin-top:6px;font-size:13px"></div>
<hr>
...
<div id="volTab" class="card">
  <h3>Volunteers <button onclick="loadVolunteers()" style="margin-left:10px;padding:6px 10px;font-size:12px">রিফ্রেশ</button></h3>
  <div id="vlist"><p>Token Save করে রিফ্রেশ চাপুন</p></div>
</div>
...
<p style="font-size:13px;color:#555;background:#fef3c7;padding:8px;border-radius:6px">
  <b>আইডি লিখে "লোড করুন" চাপলে</b> পুরনো নোট + তথ্য অটো আসবে।
</p>
...
function saveToken(){
  ...
  document.getElementById('tokenStatus').innerHTML = ADMIN_TOKEN 
    ? '<span class="success">✅ Token Saved! এখন রিফ্রেশ চাপুন</span>' 
    : '<span class="error">❌ Token খালি রাখা যাবে না</span>';
...
async function loadVolunteers(){
  ...
  if(!ADMIN_TOKEN){
    el.innerHTML = '<div class="error">❌ আগে Admin Token দিন এবং Save চাপুন</div>';
    return;
  }

  el.innerHTML = '<p>⏳ লোড হচ্ছে...</p>';
  ...
  try {
    ...
    try {
      ...
      el.innerHTML = '<div class="error">❌ Server থেকে ভুল রেসপন্স এসেছে:<br><pre style="font-size:11px;overflow:auto;max-height:150px">' + text.substring(0,400) + '</pre></div>';
      return;
    }

    if (!j.success) {
      el.innerHTML = '<div class="error">❌ ' + (j.message || 'Unauthorized - Token ভুল') + '</div>';
      return;
    }

    if (!j.data || j.data.length === 0) {
      el.innerHTML = '<p>এখনো কোনো Volunteer নেই</p>';
      return;
    }
    ...
    el.innerHTML = '<div class="error">❌ Network / Server Error: ' + err.message + '</div>';
  }
}
...
async function saveExam(){
  ...
  try{
    ...
    const j = await r.json();
    document.getElementById('examMsg').innerHTML = j.success 
      ? '<div class="success">✅ '+j.message+'</div>' 
      : '<div class="error">❌ '+j.message+'</div>';
    if(j.success) loadExamList();
  }catch(err){
    document.getElementById('examMsg').innerHTML = '<div class="error">❌ Error: '+err.message+'</div>';
  }
}
...
async function loadExamList(){
  ...
  try {
    ...
    const j = await r.json();
    if(!j.success){
      document.getElementById('examList').innerHTML = '<div class="error">❌ '+j.message+'</div>';
      return;
    }
    if(!j.data || j.data.length===0){
      document.getElementById('examList').innerHTML = '<p>কোনো রেজাল্ট নেই</p>';
      return;
    }
    document.getElementById('examList').innerHTML = '<table><tr><th>ID</th><th>নাম</th><th>বাং</th><th>ইং</th><th>ইতি</th><th>ভূগো</th><th>অংক</th><th>GK</th><th>মোট</th><th>%</th><th>Del</th></tr>' +
      j.data.map(e => {
    ...
    document.getElementById('examList').innerHTML = '<div class="error">❌ '+err.message+'</div>';
  }
}
...
async function delExam(id){
  ...
  try {
    const r = await fetch('/api/admin/results/'+encodeURIComponent(id), {method:'DELETE', headers:{'X-Admin-Token':ADMIN_TOKEN}});
    const j = await r.json();
...
async function loadLinksAdmin(){
  ...
  try {
    ...
    const j = await r.json();
    if(!j.success){
      document.getElementById('linkList').innerHTML = '<div class="error">❌ '+j.message+'</div>';
      return;
    }
    document.getElementById('linkList').innerHTML = (j.data||[]).map(x => 
      '<div style="border-bottom:1px solid #eee;padding:6px 0">'+x.title+' - <a href="'+x.link+'" target="_blank">'+x.link+'</a> <button onclick="delLink(\\''+x.id+'\\')">Delete</button></div>'
    ).join('') || '<p>কোনো লিংক নেই</p>';
  } catch(err) {
    document.getElementById('linkList').innerHTML = '<div class="error">❌ '+err.message+'</div>';
  }
}

async function addLink(){
  const title = document.getElementById('ltitle').value.trim();
  const link = document.getElementById('llink').value.trim();
  ...
  try {
    ...
    const j = await r.json();
    alert(j.message || (j.success ? 'Added' : 'Failed'));
    if(j.success) loadLinksAdmin();
  } catch(err) {
    alert('Error: ' + err.message);
  }
}
  ...
  reader.onload = async function(){
    try {
      const r = await fetch('/api/admin/upload', {
        method:'POST',
        headers:{'Content-Type':'application/json','X-Admin-Token':ADMIN_TOKEN},
        body:JSON.stringify({title: title||file.name, data: reader.result, filename: file.name})
      });
      const j = await r.json();
      if(j.success){
        document.getElementById('uploadMsg').innerHTML = '✅ সফল! <a href="'+j.link+'" target="_blank">'+j.link+'</a>';
        loadLinksAdmin();
      } else {
        document.getElementById('uploadMsg').textContent = '❌ ' + j.message;
      }
    } catch(err) {
      document.getElementById('uploadMsg').textContent = '❌ ' + err.message;
    }
  };
...
async function loadEmployeeById(){
  ...
  try{
    ...
    const j = await r.json();
    if(!j.success){ document.getElementById('empMsg').innerHTML = '<div class="error">❌ '+j.message+'</div>'; return; }
    ...
    if(found){
      ...
      document.getElementById('empMsg').innerHTML = '<div class="success">✅ পুরনো তথ্য লোড হয়েছে</div>';
      loadNoteHistory(found.emp_id);
    } else {
      document.getElementById('emp_name').value = '';
      document.getElementById('emp_post').value = '';
      document.getElementById('emp_mobile').value = '';
      document.getElementById('empMsg').innerHTML = '<div class="error">ℹ️ এই ID-তে আগে কোনো রেকর্ড নেই। নতুন হিসেবে সেভ করতে পারবেন।</div>';
      document.getElementById('noteHistory').innerHTML = '<p>এই ID-তে এখনো কোনো নোট নেই</p>';
    }
  }catch(e){
    document.getElementById('empMsg').innerHTML = '<div class="error">❌ Network Error</div>';
  }
}
...
async function saveEmployee(){
  ...
  try{
    ...
    const j = await r.json();
    document.getElementById('empMsg').innerHTML = j.success 
      ? '<div class="success">✅ '+j.message+'</div>' 
      : '<div class="error">❌ '+j.message+'</div>';
    if(j.success){
...
async function loadEmployees(){
  ...
  try {
    ...
    const j = await r.json();
    if(!j.success){
      document.getElementById('empList').innerHTML = '<div class="error">❌ '+j.message+'</div>';
      return;
    }
    if(!j.data || j.data.length===0){
    ...
    document.getElementById('empList').innerHTML = '<div class="error">❌ '+err.message+'</div>';
  }
}
...
async function loadNoteHistory(empId){
  ...
  try{
    ...
    const j = await r.json();
    if(!j.success){
      document.getElementById('noteHistory').innerHTML = '<div class="error">❌ ' + j.message + '</div>';
      return;
    }
    if(!j.data || j.data.length === 0){
    ...
    j.data.forEach(n => {
      html += '<div class="note-box"><div class="note-date">' + new Date(n.created_at).toLocaleString('bn-IN') + '</div><div>' + n.note + '</div></div>';
    });
    document.getElementById('noteHistory').innerHTML = html;
  }catch(e){
    document.getElementById('noteHistory').innerHTML = '<div class="error">❌ Error</div>';
  }
}
...
if(ADMIN_TOKEN){
  document.getElementById('tokenStatus').innerHTML = '<span class="success">✅ Token আছে — রিফ্রেশ চাপুন</span>';
  loadVolunteers();
}
calcTotal();
</script></body></html>`;
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname.startsWith("/api/volunteer/pdf/")) {
      ...
      const id = parts[4] ? decodeURIComponent(parts[4].split("?")[0]) : "";
    ...
    if (url.pathname === "/api/admin/volunteers" && req.method === "GET") {
      const token = req.headers.get("X-Admin-Token") || url.searchParams.get("token") || "";
      if (token !== env.ADMIN_TOKEN) return Response.json({ success: false, message: "Unauthorized - Token মিলছে না" }, { headers });
