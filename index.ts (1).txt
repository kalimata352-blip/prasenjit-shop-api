...
const HTML_PAGE = `<!DOCTYPE html><html lang="bn"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Volunteer Form</title>
<style>body{font-family:system-ui,sans-serif;background:#f5f5f5;margin:0;padding:12px}.card{background:#fff;max-width:650px;margin:auto;padding:15px;border-radius:8px}input,select,textarea{width:100%;padding:10px;margin:6px 0;box-sizing:border-box}button{padding:10px 15px;background:#0d6efd;color:#fff;border:0;border-radius:5px}.btn{padding:5px 8px;margin:2px;display:inline-block;text-decoration:none;background:#eee;border-radius:4px}</style></head><body>
<div class="card"><h2 style="text-align:center">ভলান্টিয়ার ফর্ম - তথ্য দিন নিচের ফর্মে</h2>
<div class="top-links"><a class="btn" href="/status">আপনার ID স্ট্যাটাস চেক করুন</a><a class="btn" href="/links">গুরুত্বপূর্ণ লিংক ও ফাইল</a></div>
<form id="f">
...
<div id="msg"></div><hr><h3>গুরুত্বপূর্ণ লিঙ্ক ও নোটিফিকেশন</h3><div id="links">লোড হচ্ছে...</div>
...
f.addEventListener('submit',async e=>{
 ...
 try{
  ...
  if(j.success){
   msg.innerHTML='<span style="color:green">✅ রেজিস্ট্রেশন সফল! আপনার ID: <b>'+j.id+'</b><br><a target="_blank" href="/api/volunteer/pdf/'+j.id+'">PDF দেখুন</a></span>';
   f.reset(); loadLinks();
...
async function loadLinks(){
 try{
  ...
  el.innerHTML=j.data.map(function(x){
   ...
   return '<div style="border:1px solid #eee;padding:10px;border-radius:8px;margin-bottom:8px"><b>'+x.title+'</b><br><span style="margin-top:6px;display:inline-block"><a href="'+x.link+'" target="_blank"'+downloadAttr+'>'+icon+' দেখুন / ডাউনলোড</a></span>'+preview+'</div>';
  }).join('');
...
<style>body{font-family:system-ui;padding:20px;background:#f5f5f5}.card{max-width:500px;margin:auto;background:#fff;padding:20px;border-radius:8px}input,button{width:100%;padding:10px;margin:6px 0}button{background:#0d6efd;color:#fff;border:0;border-radius:5px}</style></head><body><div class="card">
...
async function check(){
 ...
 const d=j.data;
 res.innerHTML='<b>নাম:</b> '+d.name+'<br><b>মোবাইল:</b> '+d.mobile+'<br><b>স্ট্যাটাস:</b> <b style="color:'+(d.status=='approved'?'green':d.status=='rejected'?'red':'orange')+'">'+d.status.toUpperCase()+'</b><br><br>'+ (d.status=='approved' ? '<span style="color:green;font-weight:bold">✅ আপনার আবেদন APPROVED হয়েছে</span>' : d.status=='rejected' ? '<span style="color:red">❌ আপনার আবেদন REJECTED হয়েছে</span>' : '<span>⏳ Pending আছে, পরে চেক করুন</span>');
}
</script></body></html>`;

const LINKS_PAGE = `<!DOCTYPE html><html lang="bn"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width"><title>Links</title>
...
async function load(){
 ...
 el.innerHTML=j.data.map(function(x){
  ...
  const downloadAttr = isPdf ? ' download' : '';
  return '<div style="border-bottom:1px solid #eee;padding:10px 0"><span><b>'+x.title+'</b></span><br><span style="margin-top:4px;display:inline-block"><a href="'+x.link+'" target="_blank"'+downloadAttr+'>লিংক খুলুন</a></span></div>';
 }).join('');
}
load();
</script></body></html>`;

const ADMIN_PAGE = `<!DOCTYPE html><html lang="bn"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width"><title>Admin</title>
<style>body{font-family:system-ui;padding:12px;background:#f9f9f9}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border:1px solid #ddd;padding:6px;font-size:13px}th{background:#eee}button{padding:4px 8px;margin:2px}</style></head><body>
<div>Admin Token: <input id="token" type="password" placeholder="Admin Token লিখুন" style="width:70%"><button onclick="saveToken()">Save</button></div>
<hr><h3>Volunteers</h3><button onclick="loadVolunteers()">🔄 রিলোড করুন</button><div id="vlist"></div><hr>
<h3>লিংক / PDF / ছবি যোগ করুন (সবার জন্য)</h3>
<input id="ltitle" placeholder="টাইটেল - যেমন: নিয়োগ বিজ্ঞপ্তি PDF" style="width:60%">
<input id="llink" placeholder="লিংক - /api/volunteer/pdf/ID বা https://..." style="width:80%">
<button onclick="addLink()">যোগ করুন</button>
<br><small style="display:block;margin:6px 0;color:#666">সরাসরি PDF আপলোড করতে চাইলে - নিচের ফর্মে ফাইল আপলোড করুন -</small>
...
function saveToken(){
 ...
 document.getElementById('token').value = ADMIN_TOKEN;
 alert('Token Saved! এখন রিলোড করুন');
}
async function loadVolunteers(){
 ...
 document.getElementById('vlist').innerHTML='<table><tr><th>ID</th><th>নাম</th><th>মোবাইল</th><th>জেলা</th><th>স্ট্যাটাস</th><th>Action</th></tr>'+j.data.map(function(v){
  return '<tr><td>'+v.id+'</td><td>'+v.name+'</td><td>'+v.mobile+'</td><td>'+(v.district||'')+'</td><td>'+v.status+'</td><td><button onclick="updateStatus(\\''+v.id+'\\',\\'approved\\')">Admit/Approve</button><button onclick="updateStatus(\\''+v.id+'\\',\\'rejected\\')">Reject</button><a target="_blank" href="/api/volunteer/pdf/'+v.id+'">PDF</a></td></tr>';
 }).join('')+'</table>';
}
...
async function loadLinksAdmin(){
 ...
 document.getElementById('linkList').innerHTML=j.data.map(function(x){
  return '<div style="border-bottom:1px solid #eee;padding:6px 0">'+x.title+' - <a href="'+x.link+'" target="_blank">'+x.link+'</a> - <button onclick="delLink(\\''+x.id+'\\')">Delete</button></div>';
 }).join('');
}
...
const PDF_PAGE = function(v: any){
 ...
 return `<!DOCTYPE html><html lang="bn"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Volunteer - ${v.id}</title>
<style>body{font-family:system-ui,sans-serif;background:#f1f1f1;padding:20px;color:#111}.card{max-width:700px;margin:auto;background:#fff;padding:20px;border-radius:8px}
h2{style:text-align:center} .row{display:flex;justify-content:space-between;border-bottom:1px solid #eee;padding:6px 0} .actions{margin-top:20px;text-align:center} .btn{padding:10px 15px;background:#0d6efd;color:#fff;border:0;border-radius:5px;margin:5px}</style></head><body>
...
export default {
 async fetch(req: Request, env: Env): Promise<Response> {
  ...
  try {
   ...
   await env.DB.prepare('CREATE TABLE IF NOT EXISTS public_links (id TEXT PRIMARY KEY, title TEXT, link TEXT, category TEXT, created_at INTEGER)').run();
  ...
  if (url.pathname === "/api/admin/upload" && req.method === "POST") {
   ...
   const id = generateId();
   await env.DB.prepare("INSERT INTO public_files (id, title, filename, data, created_at) VALUES (?,?,?,?,?)").bind(id, body.title || body.filename || id, body.filename || id, body.data, Date.now()).run();
   await env.DB.prepare("INSERT INTO public_links (id, title, link, created_at) VALUES (?,?,?,?)").bind(id, body.title || body.filename || id, '/api/file/'+id, Date.now()).run();
